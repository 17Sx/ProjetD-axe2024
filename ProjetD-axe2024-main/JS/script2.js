...
function afficherCarte(carte) {
    ...
    const carteDejaPresente = cartesInventaire.some(item => item.carte === carte.image);

    if (!carteDejaPresente) {
        // Ajout la carte à l'inventaire seulement si elle n'est pas déjà présente
        cartesInventaire.push({ carte: carte.image, favori: false });
